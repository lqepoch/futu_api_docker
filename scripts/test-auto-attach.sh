...
set timeout 10; set helper [lindex $argv 0]; set timed_out 0; spawn -noecho expect -f $helper futu-opend; expect { -re {FUTU_LOGIN_READY_MARKER} {} -re {timed out waiting for FUTU_LOGIN_READY_MARKER} {set timed_out 1} eof {} timeout {exit 124} }; expect eof; set wait_result [wait]; if {$timed_out} {exit 124}; exit [lindex $wait_result 3]
EOF
...
run_case() {
  ...
  output="$({
    env \
      ...
      timeout 10 expect -f "$test_dir/outer-harness.expect" "$helper"
  } 2>&1)"
  status=$?
  set -e
  ...
  if [[ "$mode" == marker ]]; then
    ...
    [[ -s "$sentinel" ]] || { printf 'marker sentinel missing invocation=%s\n' "$(<"$invocation")" >&2; exit 1; }
